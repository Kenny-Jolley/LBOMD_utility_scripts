...
# -----------
# This script thermalises the system to the user specified temperature with fixed boundaries.
# The size of the fixed boundaries is hard coded as this should not vary much.
# ----------
...
#  or call the function:  md_run_multi_cascades_carbon(temperature, cascade_energy, cascade_time)
#   where: temperature, cascade_energy, cascade_time,  must be positive numbers.
#
#  md_run_multi_cascades_carbon_fixed_and_thermal_bounds.py 100 1000 4000
#  md_run_multi_cascades_carbon_fixed_and_thermal_bounds.py 900 1000 4000
...
from md_setup_fixed_and_thermal_bounds import md_setup_fixed_and_thermal_bounds
from md_setup_thermalise import md_setup_thermalise
...
def md_run_multi_cascades_carbon_fixed_and_thermal_bounds(temperature,cascade_energy,cascade_time):
    
    # ---------------
    # Hard coded options
    fixed_bound_width = 2.0
    thermal_bound_width = 2.0
    output_file_freq = 10
    # ------------------
    ...
    # ensure the temperature given is a positive number
    try:
        temperature = float(temperature)
        if( (temperature < 0) ):
            print("> The temperature must be a positive number.")
            print("> Exiting ...")
            sys.exit()
    except ValueError:
        print("> The temperature must be a positive number.")
    ...
    while 1:
        ...
        if(resume):
            ...
            # ensure no interfering files from a failed run exist
            ...
            # set the boundaries
            md_setup_fixed_and_thermal_bounds(fixed_bound_width, thermal_bound_width)
            
            # thermalise the whole system
            print("> Thermalising for new casacade simulation : " + str(counter) )
            # set the output frequency so only failsafe is written at the end.
            md_setup_output_freq(100000)
            md_setup_output_failsafe(100000)
            md_setup_thermalise(temperature,1000)
            os.system("./LBOMD.exe >/dev/null")
        
            # determine if thermalisation completed successfully
            if (os.path.isfile("final-lattice.dat")):
                print("Thermalisation of cascade " + str(counter) + " completed")
                os.remove('final-lattice.dat')
            else:
                # simulation did not finish correctly
                print("Thermalisation of cascade " + str(counter) + " did not finish")
                print("Exiting ... ")
                sys.stdout.flush()
                sys.exit()
...
if __name__ == '__main__':
    ...
    md_run_multi_cascades_carbon_fixed_and_thermal_bounds(temperature,cascade_energy,cascade_time)
